...
한 곳에만 있다 — 강사구성 42(`staffScore/5×42`) · 강사료 23/16/7 · 종합판정 15/8/0 ·
하트 +12 · 간식 8/4/2, **합계 100**. 안 채운 항목은 0점, 아예 미평가면 `null`(랭킹 맨 아래),
**강사구성 0이면 무조건 0점**.

**연수는 점수에 반영하지 않는다**(참고 항목). 선택지는 라이브 / 동영상 / 초반 라이브→후반 동영상이며,
예전에 저장된 `online`/`offline` 값도 계속 허용해 사용자가 다시 고르기 전까지 사라지지 않는다. 리스트는 종합점수·강사구성·가나다·평가완료 우선으로 정렬할 수 있다.

**플래너에 표시**(`showRatings`, 기본 OFF)가 마스터 스위치다. 켜면 플래너·최근 감지 카드의
업체명 앞에 표식이 붙고, 어떤 표식을 붙일지는 개별로 고른다 —
`showVerdict`(색점) · `showStaffScore`(★) · `showHeart`(❤️) 기본 ON, `dimSkip`(거르기 기관 카드
